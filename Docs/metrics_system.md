...
The metrics system tracks performance characteristics of speculative decoding across multiple dimensions: throughput, latency, acceptance rates, network overhead, and tree-based pipeline efficiency. Metrics are collected at three levels:
...
- **Acceptance Metrics:**
  - `acceptance_ratio`: Ratio of accepted draft tokens to total drafted
...
- **Round/Token Counts:**
  ...
  - `acceptance_ratio`: Ratio of accepted to drafted tokens
- **Timing Breakdown:**
...
- `accepted_draft_per_round`: Average accepted draft tokens per round
...
3. After generation completes:
   - Computes derived metrics (acceptance ratio, mean K, etc.)
...
2. For each tree iteration:
   ...
   - Updates cumulative metrics
3. After generation completes:
   - Computes derived metrics (acceptance ratio, etc.)
...
### Acceptance Ratio
```
acceptance_ratio = total_accepted_tokens / total_drafted_tokens
```
...
for result in results:
    ...
    print(f"Acceptance rate: {result['speculative']['acceptance_rate']}")
