...
def build_profiles(base: Dict[str, Any], reuse_windows: List[int], top_k_values: List[int], include_no_update: bool) -> List[Dict[str, Any]]:
    profiles: List[Dict[str, Any]] = []
    for r in reuse_windows:
        for k in top_k_values:
            name = f"reuse_R{r}_k{k}"
            label = f"Fixed-R cloud guidance update, R={r}, top_k={k}"
            profiles.append(
                {
                    "name": name,
                    "label": label,
    ...
    if include_no_update:
        for k in top_k_values:
            profiles.append(
                {
                    "name": f"no_update_k{k}",
                    "label": f"No cloud attention refresh after initialization, top_k={k}",
...
def main() -> None:
    ...
    parser.add_argument("--top-k-values", default="8,16")
    ...
    profiles = build_profiles(base, reuse_windows, top_k_values, include_no_update=not args.no_no_update)
